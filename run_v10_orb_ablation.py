"""
V10 Opening Range Expansion (ORB) Scientific Ablation Benchmark
Isolates and evaluates the 15-minute Opening Range Breakout hypothesis under full statutory costs and slippage.

Variants Benchmarked:
  - V10-A : Pure 15m ORB (No filters)
  - V10-B : ORB + Opening Volume Confirmation (Vol >= 1.25x)
  - V10-C : ORB + VWAP Alignment (Long > VWAP, Short < VWAP)
  - V10-D : ORB + Volume + VWAP Confluence
  - V10-E : ORB + Volume + VWAP + XGBoost Probability Gate (P >= 0.40)
  - V10-F : ORB + Volume + VWAP + XGBoost + NIFTY Index Alignment
  - V10-G : Full Strategy + Dynamic ATR Risk Model

Also produces a granular Time-of-Day Sub-Window Audit:
  09:30-09:45 vs 09:45-10:00 vs 10:00-10:15 vs 10:15-10:30 vs 10:30-10:45
"""
import sys
import os
import sqlite3
...
import config
from core.v10_orb_engine import V10ORBEngine
...
def simulate_orb_variant(variant, prepared_stock_data, raw_stock_data, model=None, index_df=None):
    """
    Simulates a V10 ORB strategy variant.
    """
    labeler = TradeLabeler()
    orb_engine = V10ORBEngine()
    ...
    for symbol, df_prep in prepared_stock_data.items():
        ...
        setups = orb_engine.generate_orb_setups(
            df_prep, variant=variant, symbol=symbol, index_df=index_df
        )
    ...
    for trade in labeled_trades:
        ...
        # XGBoost Filtering for V10-E/F/G
        if variant in ['V10_E', 'V10_F', 'V10_G'] and model is not None:
        ...
        # Simulate execution with Slippage and Statutory Costs
        ...
        trade_record = {
            'trade': trade,
            'subwindow': trade.get('subwindow', '09:30-09:45'),
            'direction': direction,
            'is_win': net_pnl_rs > 0,
    ...
    # Time-of-Day Sub-Window Breakdown
    subwindows = ['09:30-09:45', '09:45-10:00', '10:00-10:15', '10:15-10:30', '10:30-10:45', '10:45+']
    subwindow_stats = {}
    for sw in subwindows:
        sw_trades = [t for t in accepted_trades if t['subwindow'] == sw]
        if sw_trades:
            sw_wins = [t for t in sw_trades if t['is_win']]
            sw_gw = sum(t['gross_pnl_rs'] for t in sw_trades if t['gross_pnl_rs'] > 0)
            sw_gl = abs(sum(t['gross_pnl_rs'] for t in sw_trades if t['gross_pnl_rs'] < 0))
            sw_net = sum(t['net_pnl_rs'] for t in sw_trades)
            sw_pf = round(sw_gw / sw_gl, 3) if sw_gl > 0 else (99.0 if sw_gw > 0 else 0.0)
            subwindow_stats[sw] = {
                'trades': len(sw_trades),
                'win_rate': round((len(sw_wins) / len(sw_trades)) * 100, 1),
                'pf': sw_pf,
                'net_pnl': round(sw_net, 2)
            }
    ...
    return {
        ...
        'subwindow_stats': subwindow_stats
    }
...
def run_orb_ablation():
    print("\n" + "=" * 115)
    print("V10 OPENING RANGE EXPANSION (ORB) SCIENTIFIC ABLATION BENCHMARK")
    print("Evaluating 7 Incremental Variants Across Core & Unseen Cohorts Under Full Statutory Costs")
    print("=" * 115)
    ...
    # 3. Index data for market alignment
    ...
    print("\n[PRECOMPUTING] Pre-processing ORB metrics...")
    core_prep = {s: orb_engine.prepare_dataset(df) for s, df in core_stock_data.items()}
    unseen_prep = {s: orb_engine.prepare_dataset(df) for s, df in unseen_stock_data.items()}
    print("[PRECOMPUTING] Ready.\n")

    variants = [
        ('V10_A', 'Pure 15m ORB (No filters)'),
        ('V10_B', 'ORB + Opening Volume Confirmation (Vol >= 1.25x)'),
        ('V10_C', 'ORB + VWAP Alignment (Long > VWAP, Short < VWAP)'),
        ('V10_D', 'ORB + Volume + VWAP Confluence'),
        ('V10_E', 'ORB + Volume + VWAP + XGBoost (P >= 0.40)'),
        ('V10_F', 'ORB + Volume + VWAP + XGBoost + NIFTY Alignment'),
    ...
    # Granular Time-of-Day Sub-Window Table for V10-D / V10-E
    print("\n" + "#" * 115)
    print("TIME-OF-DAY SUB-WINDOW AUDIT (When does the opening edge exist?)")
    print("#" * 115)
    v10d_core = next(r for r in core_results if r['variant'] == 'V10_D')
    v10d_unseen = next(r for r in unseen_results if r['variant'] == 'V10_D')

    print("\nCore Universe (V10-D):")
    for sw, stats in v10d_core['subwindow_stats'].items():
        print(f"  [{sw}] Trades: {stats['trades']:<3} | WinRate: {stats['win_rate']:>5.1f}% | PF: {stats['pf']:>5.3f} | Net PnL: Rs {stats['net_pnl']:>7.2f}")

    print("\nUnseen Universe (V10-D):")
    for sw, stats in v10d_unseen['subwindow_stats'].items():
        print(f"  [{sw}] Trades: {stats['trades']:<3} | WinRate: {stats['win_rate']:>5.1f}% | PF: {stats['pf']:>5.3f} | Net PnL: Rs {stats['net_pnl']:>7.2f}")

    # Save to disk
    out_json = os.path.join(config.RESULTS_DIR, "v10_orb_ablation_report.json")
    out_csv = os.path.join(config.RESULTS_DIR, "v10_orb_ablation_summary.csv")
